...
import prince.app.sphotos.BuildConfig;
...
public abstract class ImageWorker {
    ...
    private Bitmap loadFullImageFromSDCard(Object params){
		Bitmap bitmap = null;
		int columnIndex = 0;
    ...
    public  Bitmap loadImageThumbNailFromSDCard(Object params){
		Bitmap bitmap = null;
		Uri uri = null;  
		int columnIndex = 0;
		...
		if (cursor != null){
			...
			//	Context context = getApplicationContext();
				CharSequence text = "No Images Found!";
				int duration = Toast.LENGTH_LONG;
				Toast.makeText(mContext, text, duration).show();
				((Activity) mContext).finish(); // close the activity
			}
			...
			try {
			    ...
			    options.inSampleSize = ImageResizer.calculateInSampleSize(options, 110, 110);
			    if (Global.hasHoneycomb()) {
		            ImageResizer.addInBitmapOptions(options, mImageCache);
		        }
			...
			//	return loadImageThumbNailFromSDCard(params);
			}
		}
     ...
     */
    public void loadImage(Object data, ImageView imageView, String loadImageID) {
     ...
     //       Log.d(TAG, "Bitmap found in Cache");
            ...
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView, loadImageID);
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, BitmapDrawable> {
        ...
        private final WeakReference<String>loadID;

        public BitmapWorkerTask(ImageView imageView, String loadImageID) {
            imageViewReference = new WeakReference<ImageView>(imageView);
            loadID = new WeakReference<String>(loadImageID);
        }
        ...
        //        Log.d(TAG, "doInBackground - starting work");
          ...
          //      Log.d(TAG, "Bitmap found in Disk");
            }
            ...
            if (bitmap == null && !isCancelled() && getAttachedImageView() != null
                    && !mExitTasksEarly) {
            	if (loadID.get().equalsIgnoreCase(Util.LOCALPHOTO_THUMBNAIL_LOAD_ID)){
            		bitmap = loadImageThumbNailFromSDCard(params[0]);
            	}
            	else if (loadID.get().equalsIgnoreCase(Util.LOCALPHOTO_FULLIMAGE_LOAD_ID)){
            		bitmap = loadFullImageFromSDCard(params[0]);
            	}
            	else
