...
public class FBMainActivity extends ActivityAccounts implements FragmentX.FragmentXListener{
	...
	//	if (BuildConfig.DEBUG) Global.getInstance().modPref(UpdateFiles.LAST_GRID_IMAGE_UPDATE, "20140910");
        ...
        fragments[LOGIN] = FacebookLogin.newInstance();//fm.findFragmentById(R.id.splashFragment);
        fragments[MAIN] = Fragment_Main.newInstance(); //fm.findFragmentById(R.id.selectionFragment);
        fragments[LOGOUT] = new UserSettingsFragment(); //fm.findFragmentById(R.id.userSettingsFragment);

        FragmentTransaction transaction = fm.beginTransaction();
        for(int i = 0; i < fragments.length; i++) {
        	transaction.add(R.id.frame_fb_main_activity, fragments[i]);
